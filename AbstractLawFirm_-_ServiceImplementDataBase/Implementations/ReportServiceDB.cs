...
namespace AbstractLawFirm___ServiceImplementDataBase.Implementations
{
    public class ReportServiceDB : IReportService
    {
        ...
        public ReportServiceDB(AbstractLawFirmDbContext context)
        {
            this.context = context;
        }
        public void SaveDocumentPrice(ReportBindingModel model)
        {
            ...
            try
            {
               ...
               missing);
                ...
                var products = context.Documents.ToList();
                //создаем таблицу
               ...
               missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = products[i].DocumentsName;
        ...
        public List<ArchivesLoadViewModel> GetArchivesLoad()
        {
            return context.Archives
            .ToList()
           .GroupJoin(
            context.ArchiveComponents
            .Include(r => r.Blank)
           .ToList(),
            stock => stock,
           stockComponent => stockComponent.Archive,
           (stock, stockCompList) =>
            new ArchivesLoadViewModel
            {
                ArchiveName = stock.ArchiveName,
                TotalCount = stockCompList.Sum(r => r.Count),
                Blanks = stockCompList.Select(r => new Tuple<string,
    int>(r.Blank.BlankName, r.Count))
            })
            .ToList();
        }
        ...
        public List<CustomerOrdersViewModel> GetCustomerOrders(ReportBindingModel model)
        {
           ...
           rec.DateCreate),
                DocumentName = rec.Documents.DocumentsName,
                Count = rec.Count,
                Sum = rec.Sum,
        ...
        public void SaveCustomerOrders(ReportBindingModel model)
        {
           ...
           iTextSharp.text.Font.BOLD);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].DocumentName, fontForCells));
                table.AddCell(cell);
